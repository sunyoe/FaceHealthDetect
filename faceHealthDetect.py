...
def faceDetect(img, flag):
    cascPath = "D:\MachineLearning\OpencvPackage\install\etc\haarcascades\haarcascade_frontalface_default.xml"
    eyePath = "D:\MachineLearning\OpencvPackage\install\etc\haarcascades\haarcascade_eye.xml"
    smilePath = "D:\MachineLearning\OpencvPackage\install\etc\haarcascades\haarcascade_smile.xml"
    ...
    for(x, y, w, h) in faces:
        ...
        cv.imwrite('facePart.jpg', face_part)
        ...
        # 生成结果图片
        cv.imwrite('DiagnoseResult.jpg', img)
...
def spotDefect(img):
    ...
    face_with_spots = cv.drawKeypoints(spot_det, spotPoints, np.array([]), (255, 0, 0), cv.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
...
def wordCreate(name, gender, faceColor, faceGloss, SkinResults, GlossResults, image):
    ...
    doc.save('FaceDiagnoseResults.docx')
